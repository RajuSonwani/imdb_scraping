import os,json,requests,time,random,task1,task12
from bs4 import BeautifulSoup
from pprint import pprint

def movie_detailsLst(movies,cast):
	movie_details_list=[]
	for movie in movies:
		link0=movie["link"]
		movieId=link0[-10:-1]
		...
		if os.path.exists("/home/navgurukul/Desktop/rAju/data/task0/"+link0[-10:-1]+".json"):
			with open("/home/navgurukul/Desktop/rAju/data/task0/"+link0[-10:-1]+".json") as file:
				# tempDict["cast"]=cast[movieId]
				# movie_details_list.append(json.loads(file.read()))
				# movie_details_list.append(tempDict)
				dict0=json.loads(file.read())
				dict0["cast"]=cast[movieId]
				movie_details_list.append(dict0)
...
# pprint(movie_detailsLst(moviesLst,cast))
